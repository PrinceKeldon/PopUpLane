...
export const MinimalistHero = ({
  ...
  return (
    ...
    >
      ...
      <div className="relative grid w-full max-w-7xl flex-grow grid-cols-1 items-center md:grid-cols-3">
        ...
        <div className="relative order-1 md:order-2 flex justify-center items-center h-full">
            ...
            <motion.img
                ...
                target.onerror = null;
                target.src = `https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?w=400`;
                }}
            />
        </div>
      ...
      {/* Countdown Timer Section (if provided) */}
      {countdownComponent && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 1.4 }}
          className="z-30 w-full max-w-7xl mb-8"
        >
          {countdownComponent}
        </motion.div>
      )}
      ...
      <footer className="z-30 flex w-full max-w-7xl items-center justify-between">
        ...
        >
          {socialLinks.map((link, index) => (
            <SocialIcon key={index} href={link.href} icon={link.icon} onClick={link.onClick} />
          ))}
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
