...
const ShopperSignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API}/shopper/login`, formData);
      
      localStorage.setItem('shopper_token', response.data.token);
      localStorage.setItem('shopper_account', JSON.stringify(response.data.shopperAccount));
      ...
      <div className="w-full max-w-md px-6">
        ...
        <Card className="p-8" style={{ backgroundColor: '#FAFAFA' }}>
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif', color: '#111' }}>Welcome Back</h2>
            <p style={{ color: '#666' }}>Sign in to access your saved finds</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <Input id="password" name="password" type="password" value={formData.password} onChange={handleChange} required className="mt-2" />
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm" style={{ color: '#666' }}>
              New here? <button onClick={() => navigate('/shopper/register')} className="font-semibold" style={{ color: '#FF4F81' }}>Create account</button>
            </p>
          </div>
        </Card>
      </div>
